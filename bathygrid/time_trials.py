...
def trial_data():
    ...
    densitygrid = np.full((100, 100), 0)
    grid = np.full((100, 100), np.nan)
    vuncgrid = np.full((100, 100), np.nan)
    huncgrid = np.full((100, 100), np.nan)
    return depth, vunc, hunc, cell_indices, grid, densitygrid, vuncgrid, huncgrid
...
def trial_grid_mean_numpy():
    depth, tvu, thu, cell_indices, grid, densitygrid, tvugrid, thugrid = trial_data()
    np_grid_mean(depth, cell_indices, grid, densitygrid, tvu, thu, tvugrid, thugrid)
...
def trial_grid_shoal_numpy():
    depth, tvu, thu, cell_indices, grid, densitygrid, tvugrid, thugrid = trial_data()
    np_grid_shoalest(depth, cell_indices, grid, densitygrid, tvu, thu, tvugrid, thugrid)
